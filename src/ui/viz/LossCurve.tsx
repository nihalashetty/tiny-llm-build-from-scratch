...
export function LossCurve({
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width="100%" role="img" aria-label="Training loss over time">
      <rect x={pad.l} y={pad.t} width={w} height={h} fill="#fff" stroke="#eae0d3" />
      {yTicks.map((t, i) => {
        const y = pad.t + h - (t / yMax) * h;
        return (
          <g key={i}>
            <line x1={pad.l} y1={y} x2={pad.l + w} y2={y} stroke="#f0e7d8" />
            <text x={pad.l - 6} y={y + 3} textAnchor="end" fontFamily="'JetBrains Mono', monospace" fontSize="9" fill="#a2917a">
              {t.toFixed(2)}
            </text>
          </g>
        );
      })}
      {n > 1 && <polyline points={pts.join(' ')} fill="none" stroke="#f0663e" strokeWidth="2" />}
      <text x={pad.l} y={height - 6} fontFamily="'JetBrains Mono', monospace" fontSize="9" fill="#a2917a">
        training time →
      </text>
    </svg>
  );
